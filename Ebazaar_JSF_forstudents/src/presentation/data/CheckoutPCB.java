...
public class CheckoutPCB implements Serializable {
    ...
    private CheckoutController usecaseControl = CheckoutController.INSTANCE;
    @Inject
    ...
    public List<Address> getAddressList() {
        CustomerSubsystem cust = sessionContext.getCust();
        try {
            addressList = usecaseControl.getShippingAddresses(cust.getCustomerProfile());
    ...
    public CreditCard getCard() {
    	LOG.warning("Fake default credit card is being used");
    	return CustomerSubsystemFacade.createCreditCard("Test name",
				"00/00/0000", "0000000000000000", "Master Card");
    ...
    public List<CartItem> getCartItems() {
        CustomerSubsystem cust = sessionContext.getCust();
        return cust.getShoppingCart().getCartItems();
    }
    ...
    public String proceedToPaymentWindow(String currentPage) {
        ...
        try {
            if (newShippingAddress) {
                shippingAddress =
                        usecaseControl.runAddressRules(shippingAddress);
                if (billingAddSameAsShippingAdd) {
                    billingAddress = shippingAddress;
                }
            }
    ...
    public String proceedToTermsConditions(String currentPage) {
        ...
        try {
            usecaseControl.runPaymentRules(billingAddress, card);
            usecaseControl.verifyCreditCard();
